...
import { Phone, Clock, CheckCircle, AlertCircle, Loader2, RefreshCw } from 'lucide-react';
...
const DialerQueueMonitor: React.FC<DialerQueueMonitorProps> = ({
  ...
  const fetchQueueStats = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('session_queue_stats').select('*').eq('session_id', sessionId).maybeSingle();
      ...
      if (data) {
        ...
        const {
          data: leadsData,
          error: leadsError
        } = await supabase.from('dialing_session_leads').select('status, lead_id, notes').eq('session_id', sessionId);
  ...
  useEffect(() => {
    if (sessionId) {
      ...
      const channel = supabase.channel('queue_changes').on('postgres_changes', {
        event: '*',
        schema: 'public',
