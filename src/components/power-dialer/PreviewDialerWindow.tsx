...
import { Phone, UserX, PhoneOff, MessageSquare, Ban, PhoneMissed, Clock, RotateCcw, Pause, StopCircle, Play } from 'lucide-react';
...
const PreviewDialerWindow: React.FC<PreviewDialerWindowProps> = ({
  ...
  const fetchCallingLists = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('get-calling-lists');
      if (error) {
  ...
  const handleBeginDialing = async () => {
    ...
    try {
      ...
      console.log("Starting dialing session with:", {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('start-dialing-session', {
        body: {
          listId: selectedListId,
          sessionName
        }
  ...
  const saveCallNotes = async () => {
    ...
    try {
      ...
      const {
        data: refreshedNotes
      } = await supabase.from('lead_notes').select('*').eq('lead_id', currentLead.id).order('created_at', {
        ascending: false
      });
      if (refreshedNotes) {
  ...
  return <>
      <Card className="bg-gray-800 p-4 rounded-lg">
        <div className="grid grid-cols-3 gap-4">
          {[1, 2, 3].map(line => <LineDisplay key={line} lineNumber={line} currentCall={activeCallsForDisplay[line]} />)}
        </div>
      </Card>

      <div className="grid grid-cols-4 gap-0">
        <div className="col-span-3 pr-4">
          {!isDialingStarted ? <Card>
              ...
              <CardContent>
                <div className="flex flex-col items-center justify-center py-12">
                  <Button onClick={() => setIsDialingStarted(true)} className="bg-green-500 hover:bg-green-600 text-white px-8 py-6 text-lg rounded-lg flex items-center gap-3">
                    <Play className="h-6 w-6" />
                    Start Dialing
                  </Button>
                </div>
              </CardContent>
            </Card> : currentCall && currentCall.status === 'in-progress' ? <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-medium flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Phone className="h-5 w-5 text-green-500" />
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        {currentLead?.phone1 && <div className="flex items-center gap-2 text-sm text-gray-600">
                            <Phone className="h-4 w-4 text-gray-400" />
                            {formatPhoneNumber(currentLead.phone1)}
                          </div>}
                        
                        {currentLead?.email && <div className="flex items-center gap-2 text-sm text-gray-600">
                            <Mail className="h-4 w-4 text-gray-400" />
                            {currentLead.email}
                          </div>}
                        
                        {currentLead?.property_address && <div className="flex items-start gap-2 text-sm text-gray-600 col-span-2">
                    ...
                    <div className="flex items-center gap-2">
                      <HangupButton callSid={currentCall?.callSid || currentCall?.parameters?.CallSid} onSuccess={onEndCall} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    <div className="space-y-2">
                      ...
                      <textarea className="w-full h-32 p-2 border rounded-md text-sm" placeholder="Enter call notes here..." value={callNotes} onChange={handleCallNotesChange} />
                      <Button variant="default" size="sm" onClick={saveCallNotes} disabled={!callNotes.trim()}>
                        Save Notes
                      </Button>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <ScrollArea className="h-40 border rounded-md p-2">
                        {leadNotes.length === 0 ? <p className="text-sm text-gray-500 text-center py-4">No notes available</p> : <div className="space-y-2">
                            {leadNotes.map((note, index) => <div key={note.id || index} className="text-sm p-2 bg-gray-50 rounded">
                  ...
                  <div className="mt-4 border-t pt-4">
                    ...
                    <DispositionSelector currentDisposition={currentLead?.disposition || 'Not Contacted'} onDispositionChange={disposition => {
                  if (currentLead?.id) {
                    leadProfileService.updateDisposition(currentLead.id, disposition).then(() => {
                      setCurrentLead({
                        ...currentLead,
                        disposition
                      });
              ...
              <CardContent>
                <div className="space-y-4">
                  {sessionId && <>
                      ...
                      {!autoDialerActive && <div className="flex justify-center my-4">
                          <Button onClick={handleStartPowerDialing} className="bg-green-500 hover:bg-green-600 text-white px-8 py-4 text-lg rounded-lg flex items-center gap-3" disabled={isCreatingSession || isProcessingCall}>
                            {isProcessingCall ? <Loader2 className="h-5 w-5 animate-spin" /> : <Phone className="h-5 w-5" />}
                            Start Power Dialing
                          </Button>
                        </div>}

                      <AutoDialerController sessionId={sessionId} isActive={autoDialerActive} onCallComplete={handleCallComplete} />
                      
                      <ConnectedLeadPanel leadData={currentLead} />
                    </>}
                  ...
                  {sessionId ? <div className="text-center py-6">
                    ...
                    </div> : selectedListId && <div className="mb-4 flex justify-center">
                      <Button onClick={handleBeginDialing} className="bg-crm-blue hover:bg-crm-blue/90 text-white px-8 py-4 text-lg rounded-lg flex items-center gap-3" disabled={isCreatingSession}>
                        {isCreatingSession ? <Loader2 className="h-5 w-5 animate-spin" /> : <Phone className="h-5 w-5" />}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {selectedListId && !sessionId && <Button variant="outline" onClick={() => setSelectedListId(null)} className="text-sm" disabled={isCreatingSession}>
                        Change List
                      </Button>}
                  </div>
                  
                  {isLoadingLists ? <div className="text-center py-8 text-gray-500">
                      <Loader2 className="h-8 w-8 animate-spin mx-auto mb-2 text-gray-400" />
                      Loading calling lists...
                    </div> : callingLists.length === 0 ? <div className="text-center py-8">
                      ...
                      {callingLists.map(list => <Card key={list.id} className={`
                          ...
                          <CardContent className="p-4">
                            <div className="flex justify-between items-center">
                              ...
                              {selectedListId === list.id && <Badge variant="outline">
                                  Selected
                                </Badge>}
                            </div>
                          </CardContent>
                  ...
                  {selectedListId && !sessionId && <div className="mt-6 space-y-4">
                      <LeadSelectionPanel listId={selectedListId} onLeadsSelected={leads => {
