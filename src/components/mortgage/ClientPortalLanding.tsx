      ...
      if (slug && token && !isValidating && !isLoading) {
        try {
          const {
            access,
            error
          } = await getPortalAccess(slug, token);
          if (access) {
      ...
      if (accessToken) {
        ...
        try {
          const {
            access,
            error
          } = await getPortalAccess(slug, accessToken);
          if (access) {
            ...
            if (access.lead_id) {
              const {
                data: leadData,
                error: leadError
              } = await supabase.from('leads').select('is_mortgage_lead, added_to_pipeline_at').eq('id', access.lead_id).single();
  ...
  return <div className="min-h-screen">
      <nav className="absolute top-0 left-0 right-0 z-50 py-6 px-4">
        <div className="max-w-6xl mx-auto">
          <ul className="flex justify-center space-x-8">
            {navigationOptions.map(option => <li key={option.label}>
                <button onClick={() => navigate(option.path)} className="px-6 py-2 rounded-full text-white transition-all duration-300 hover:bg-white hover:text-blue-900 font-medium">
                  {option.label}
                </button>
              </li>)}
          </ul>
      ...
      <section className="pt-56 md:pt-72 pb-24 px-4 bg-gradient-to-br from-blue-950 to-blue-900">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-24">
            <GlowingCard title="Secure Access" description="Bank-level security protects your sensitive information while providing easy access to your documents." gradient="bg-purple-glow" icon={<Shield className="h-6 w-6 text-blue-200" />} delay="delay-1" />

            <GlowingCard title="Real-Time Updates" description="Track your loan's progress in real-time and get instant notifications about important milestones." gradient="bg-green-glow" icon={<PieChart className="h-6 w-6 text-blue-200" />} delay="delay-2" />

            <GlowingCard title="Document Management" description="Upload, sign, and manage all your loan documents in one centralized, easy-to-use platform." gradient="bg-blue-glow" icon={<FileCheck className="h-6 w-6 text-blue-200" />} delay="delay-3" />
          </div>
        </div>
      </section>
            ...
            <div className="lg:col-span-8 bg-[#F1F0FB] rounded-3xl p-16 shadow-xl min-h-[800px]">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="md:col-span-1 space-y-8">
                  <InfoCard title="24/7 Support, Even on Weekends" description="Text, call, or email your loan advisor 7 days a week. Day or night, on your schedule and receive an instant response." />
                  <InfoCard title="AI-Powered Rate Shopping" description="We use AI to analyze the top mortgage lenders in the nation to ensure we are shopping for the best possible rate and closing costs." />
                </div>

                <div className="md:col-span-1 mt-0">
                  <InfoCard title="Stress-Free Mortgage Process" description="No more stress involved in the mortgage process! No more dealing with a loan officer - our AI agent does all of the work for you!" />
                </div>
              </div>
            </div>
      ...
      <section className="py-20 px-4 bg-gradient-to-br from-blue-900 to-blue-950">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <Button size="lg" onClick={handleEnterPortal} className="bg-blue-600 hover:bg-blue-700 text-white" disabled={isValidating}>
