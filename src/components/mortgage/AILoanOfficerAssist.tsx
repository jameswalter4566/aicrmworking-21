...
const AILoanOfficerAssist = ({ leadId }: AILoanOfficerAssistProps) => {
  ...
  const copyPortalLink = () => {
    if (generatedPortalLink) {
      navigator.clipboard.writeText(generatedPortalLink);
      toast.success("Portal link copied to clipboard");
    } else {
      toast.error("No portal link available to copy");
    }
  };

  // Function to handle when a link is generated by the ClientPortalGenerator
  const handlePortalLinkGenerated = (url: string) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Borrower Portal Section - Only show if we have a generated link */}
      {generatedPortalLink && (
        <Card className="bg-white border border-blue-100">
          <CardHeader className="bg-blue-50 pb-2">
            <CardTitle className="text-lg font-medium text-blue-900">
          ...
          <CardContent className="pt-4">
            <div className="p-4 border border-blue-100 rounded-md bg-blue-50">
              ...
              <div className="flex items-center text-sm text-blue-800 bg-white p-2 rounded border border-blue-200 overflow-x-auto">
                ...
                <span className="truncate">{generatedPortalLink}</span>
              </div>
