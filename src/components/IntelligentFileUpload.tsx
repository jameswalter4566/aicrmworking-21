...
const IntelligentFileUpload: React.FC<IntelligentFileUploadProps> = ({
            ...
            if (Object.keys(headerMap).length < 2) {
              ...
              Object.entries(fieldMappings).forEach(([fieldName, variations]) => {
                if (headerMap[fieldName] === undefined) {
                  const matchIndex = headers.findIndex(header => variations.some(variation => header.toLowerCase().includes(variation) || variation.includes(header.toLowerCase())));
                  if (matchIndex >= 0) {
  ...
  return <div className="upload-container">
      {isProcessing ? <div className="space-y-4 p-6 text-center">
          <div className="mb-4">
        ...
        </div> : uploadStatus === 'success' ? <div className="p-8 text-center">
        ...
        </div> : uploadStatus === 'error' ? <div className="p-8 text-center">
          ...
          <div className="border-2 border-dashed rounded-lg p-8 text-center cursor-pointer" onClick={() => document.getElementById('file-upload')?.click()} onDragEnter={handleDragEnter} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
