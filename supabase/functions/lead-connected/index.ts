...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Modified lookup logic to handle both UUID and original IDs
    ...
    if (dialingLead) {
      console.log(`Found in dialing_session_leads with ID: ${dialingLead.id}, original_lead_id: ${dialingLead.original_lead_id}`);
      
      if (dialingLead.original_lead_id) {
        const { data: actualLead } = await supabase
          ...
          .select(`
            ...
            updated_at
          `)
          .eq('id', dialingLead.original_lead_id)
          .maybeSingle();
          ...
          if (callData?.callSid) {
            await supabase
              ...
              .upsert({
                ...
                lead_id: leadId,
                original_lead_id: dialingLead.original_lead_id,
